"""Immowelt / Immonet - bot-defended, terms of service restrict automated access.

**This adapter is disabled by default** (``enabled: false`` in
config/sources.yaml) and is meant only for personal, low-rate use run from
your own machine and your own IP.
...
def build_search_url(base_url: str, query: str, *, page: int = 1) -> str:
    ...
    params = f"sort=relevanz&term={quote(query)}"
    if page > 1:
        params += f"&page={page}"
    return f"{base}/classified-search?{params}"
...
def parse_search_results(html: str, *, source_key: str, base_url: str) -> list[RawListing]:
    ...
    cards = tree.css('[class*="EstateItem"]') or tree.css('[class*="listitem"]')
    for card in cards:
...
class ImmoweltAdapter(SourceAdapter):
