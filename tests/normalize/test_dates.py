...
def test_iso_week():
    ...
    assert result == datetime(expected.year, expected.month, expected.day, tzinfo=timezone.utc)
...
def test_iso_string_with_z_suffix():
    ...
    assert result == datetime(2026, 3, 12, 10, 30, tzinfo=timezone.utc)


def test_iso_string_with_offset_converts_to_utc():
    result = parse_german_date("2026-03-12T10:30:00+02:00")
    assert result == datetime(2026, 3, 12, 8, 30, tzinfo=timezone.utc)
