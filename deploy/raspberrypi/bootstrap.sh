...
printf 'HOFRADAR_PASSWORD_HASH=%s\n' "$hash" >> "$tmp"
...
if [ -n "$generated_pw" ]; then
  ...
  sed -i "s|^HOFRADAR_PASSWORD_HASH=.*|HOFRADAR_PASSWORD_HASH=${new_hash}|" "$APP_DIR/.env"
...
if [ "$RUNTIME" = native ]; then
  systemctl enable --now hofradar-scheduler.service
fi
